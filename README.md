...
- Model-driven SkillApps ([docs/app.md](docs/app.md)): restricted, statically compiled `~/.marifold/apps/<name>/skillapp.ts` templates, app-local or profile-installed Skills, explicit or profile-inherited models, semantic form and attachment components, static fail-closed read permissions, service-owned state, debounced latest triggers, structured results, and bookmarkable `/apps/<name>` views with an Activity drawer.
