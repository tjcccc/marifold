...
- Agent execution: optional model-generated plan, approval-aware tool loop, focused checks through normal tools, and a task summary persisted as ephemeral task state. User messages reach the model unchanged; runtime and planning guidance stays in separate instruction context. There is no separate model self-grading pass.
