...
**v0.75.0 workspace status: in testing.** Device-hosted workspaces and the Linux bridge installer are available for trials. An Aliyun ECS bridge over public HTTPS and paired Mac profile/avatar/session reads have been verified. Reboot recovery and broader host–guest acceptance remain pending. Original-file downloads have an opt-in [WebRTC experiment](docs/workspaces.md#experimental-direct-file-downloads) with bridge fallback. See [workspace setup](docs/workspaces.md) and [bridge hosting](apps/bridge/HOSTING.md).
...
- Agent execution: optional model-generated plan, approval-aware tool loop, focused checks through normal tools, and a task summary persisted as ephemeral task state. User messages reach the model unchanged; runtime and planning guidance stays in separate instruction context. Per-turn environment context identifies the provider and requested model selected by Marifold, so the assistant can report its configured model without guessing; it is not a verification of provider-side aliases. There is no separate model self-grading pass.
...
Marifold enables web search by default: supported provider/model connections use native search first; otherwise the experimental built-in backend fetches DuckDuckGo HTML, with Brave HTML as a bounded fallback. No search-service account or API key is required, but queries still leave the device for public search engines. Explicit DuckDuckGo, Firecrawl, and Ollama Cloud configurations remain supported. Set `[web_search].enabled = false` to disable both native and fallback search. See [built-in search and evaluation](docs/web-search.md) for limits and benchmark instructions.
...
[web_search]
enabled = true
provider = "builtin" # experimental, no API key
max_results = 5
...
The service refreshes expiring OAuth credentials before live provider model listing, including xAI. Model-list requests use the provider's configured proxy. An HTTP 403 response means the request received a refusal; check credential access or reauthenticate if it persists after refresh.
