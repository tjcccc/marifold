...
- Model-driven SkillApps ([docs/app.md](docs/app.md)): restricted, statically compiled `~/.marifold/apps/<name>/skillapp.ts` templates, app-local Skills, explicit models, semantic form components, service-owned state, debounced latest triggers, structured results, and a dedicated Apps view with an Activity drawer.
...
- Native-first web search: supported models (currently ChatGPT Responses) use provider-hosted search independently of `[web_search].enabled`; other models fall back to Marifold's `web_search` tool when enabled. Runs with neither capability tell the model that search is unavailable. `read_file` remains a caller-executed chat tool when the fallback section is enabled.
- File attachment through chat `/read <path>` (100k-char truncation) and image attachment through `/image <path>` / `/image clear` and `ask --image <path>`. Local/base64 images are validated, MIME-corrected, metadata-stripped, and optimized before provider requests (1600 px maximum long edge; lossless WebP for PNG/transparent inputs; conservative high-quality encoding for JPEG and oversized static WebP inputs; animated images preserved). The candidate is used only when smaller. TUI and Web UI support one-turn `/attach-original <prompt>` to preserve original encoded bytes. Embedded/URL images are retained as display-only session attachments so the Web UI can restore transcript thumbnails without adding them to later model context. The Web composer extracts readable text locally from modern Word (`.docx`), Excel (`.xlsx`), and PowerPoint (`.pptx`) files and accepts other bounded binaries for agent runs. Every agent upload is staged read-only under the private run input directory and exposed by opaque ID through `inspect_attachment`; images reach the model only when inspected, Office previews return their extracted text, and unsupported binaries return metadata plus their run-scoped path for format-specific local tools.
...
- Native provider tool calling via `@priest-ai/core` 3.x for Ollama, OpenAI-compatible Responses (including the GitHub Copilot path), and Anthropic providers, plus provider-executed web search on the ChatGPT Responses route. Neutral reasoning configuration, safe provider summaries, opaque multi-turn reasoning continuity, and cached/reasoning token usage are preserved across chat and agent tool loops.
...
Marifold fallback search uses DuckDuckGo scraping by default, which requires no API key but can be blocked by DuckDuckGo's anomaly detection on some networks. Errors surface clearly in `/search` output and tool results, and the `SearchBackend` interface is pluggable for alternative engines. Provider-hosted search is selected first when the active adapter supports it and is unaffected by the fallback toggle.
