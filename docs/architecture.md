...
- `ToolRegistry` holds effectful `AgentTool` implementations (`inspect_attachment`, `read_file`, `write_file`, isolated `shell_exec`, per-run `python_package_install`, and depth-1 `ask_profile`) plus the interaction-only `ask_user` tool. `inspect_attachment` accepts only opaque IDs from the current run manifest: inspected images become multimodal input for the remainder of that run, readable/extracted documents return bounded text, and other binaries reveal only metadata plus their scoped input path. Effectful tools may flag risky calls (`assessRisk`), which forces approval; `ask_user` instead emits a bounded, validated question request and waits for one complete submission without entering the permission system.
...
Chat resolves search in three states: provider-hosted search when the adapter supports it, Marifold's `WebSearchTool` fallback when `[web_search].enabled`, or an explicit unavailable instruction when neither exists. Hosted search does not enter Marifold's caller tool loop or emit its per-call approval events; the effective agent `network` policy can disable it, and unattended runs require explicit network `allow`. `/search` still calls the pluggable `SearchBackend` directly, and fallback `web_search`/`read_file` exchanges remain turn-local — sessions store only the prompt and final answer, and memory payloads apply only from the final response.
